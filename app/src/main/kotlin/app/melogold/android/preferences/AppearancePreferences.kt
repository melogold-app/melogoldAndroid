...
import app.melogold.android.preferences.OldPreferences.ColorPaletteMode
import app.melogold.android.preferences.OldPreferences.ColorPaletteName
...
object AppearancePreferences : GlobalPreferencesHolder() {
     ...
     * default, "color of the playing track") → the platform default; the artwork colors now live in
     * [artworkColorScope].
     */
...
private fun colorSourceOf(stored: String?): ColorSource = when (stored) {
    null -> when (OldPreferences.oldColorPaletteName) {
        ColorPaletteName.Default, ColorPaletteName.PureBlack -> ColorSource.Brand
        ColorPaletteName.MaterialYou -> ColorSource.System
        ColorPaletteName.Dynamic, ColorPaletteName.AMOLED -> defaultColorSource
    }
