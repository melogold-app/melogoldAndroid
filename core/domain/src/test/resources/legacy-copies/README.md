...
- **Файлы:** байты не менялись, это ровно то, что записало приложение. У всех трёх в заголовке `journal_mode = wal`: Room экспортирует файл в WAL-режиме, без файлов `-wal` и `-shm`. Импортёр переключает свою копию на `DELETE` (§4.5.2, п. 1).
